...
function NotebookEditToolDiffInner(t0) {
    ...
    if ($[23] !== cell_type || $[24] !== edit_mode || $[25] !== hunks || $[26] !== new_source || $[27] !== notebook_path || $[28] !== oldSource || $[29] !== width) {
        t9 = edit_mode === "delete" ? _jsx(Box, { flexDirection: "column", paddingLeft: 2, children: _jsx(HighlightedCode, { code: oldSource, filePath: notebook_path }) }) : edit_mode === "insert" ? _jsx(Box, { flexDirection: "column", paddingLeft: 2, children: _jsx(HighlightedCode, { code: new_source, filePath: cell_type === "markdown" ? "file.md" : notebook_path }) }) : hunks ? intersperse(hunks.map(_ => _jsx(StructuredDiff, { patch: _, dim: false, width: width, filePath: notebook_path, firstLine: new_source.split("\n")[0] ?? null, fileContent: oldSource }, _.newStart)), _temp3) : _jsx(HighlightedCode, { code: new_source, filePath: cell_type === "markdown" ? "file.md" : notebook_path });
        $[23] = cell_type;
        $[24] = edit_mode;
        $[25] = hunks;
        $[26] = new_source;
